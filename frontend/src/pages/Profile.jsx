// ✅ ملف Profile.jsx المعدل بالكامل
// - لا يُفقد شيء عند الضغط على Save بدون تعديل
// - الصور تبقى كما هي إذا لم يتم تعديلها
// - تظهر أزرار تغيير الصورة بشكل واضح
// - الحجم يبقى ثابتًا للصور
...
const Profile = () => {
  ...
  const [backgroundImage, setBackgroundImage] = useState("/src/assets/relax.png");
    ...
    try {
     ...
     formData.append("showPhone", editedProfile.showPhone ? "true" : "false");
      ...
      if (profileImageFile) {
        ...
        formData.append("existingProfileImage", getImageName(profile.profileImage));
      }

      if (backgroundImageFile) {
        formData.append("backgroundImage", backgroundImageFile);
      } else {
        formData.append("existingBackgroundImage", getImageName(backgroundImage));
      }
...
setIsEditing(false);
  ...
  const fetchProfile = async () => {
    try {
      ...
      console.log("✅ CreatedAt from API:", parsed.createdAt);
...
>
  ...
  <option value="Ma'an">Ma'an</option>
              ...
              <p>
                <Phone />
                {isOwnProfile ? profile.phone : profile.showPhone ? profile.phone : null}
       ...
       {activeTab === "donations" && (
            <div className="row mt-4">
              {userDonations.map((donation) => (
                <div key={donation._id} className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
                <div
  className="card shadow-sm border-0 position-relative h-100"
...
>
                    ...
                    <div className="card-body">
                      ...
                      {isOwnProfile && (
                        <button
                          className={`btn btn-sm ${donation.isValid === false ? 'btn-secondary' : 'btn-success'}`}
                          onClick={async () => {
                            try {
...
{activeTab === "saved" && (
  <div className="row mt-4">
    {savedDonations.length ? savedDonations.map((don) => (
      <div key={don._id} className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
      <div className="saved-donation-card shadow-sm border-0 position-relative"

          onClick={() => navigate(`/donations/${don._id}`)}
        >
          <img
            src={don.images?.[0] ? `http://localhost:5050/uploads/${don.images[0]}` : "/assets/placeholder.png"}
            className="card-img-top saved-donation-img"
            alt={don.title}
          />
